  ...
  <body>
          ...
          <div class="modal-body">
            <form id="RegisterValidation" enctype="multipart/form-data">
              ...
              <div class="form-group row mb-3">
                ...
                <div class="col-sm-9">
                  <textarea id="deskripsi" name="deskripsi" class="form-control" rows="5" placeholder="Deskripsi Alat jika ada"></textarea>
                  <span class="help-block"></span>
                </div>
              </div>
              <div class="form-group row mb-3">
    ...
    <script>
      ...
      function openModal() {
        ...
        $('.modal-title').text('Tambah Fasilitas Uji'); // Set Title to Bootstrap modal title
      }

      function closeModal(){
        $('#RegisterValidation')[0].reset();
        $('#modal').modal('hide');
        $('.modal-title').text('Tambah Fasilitas Uji');
      }
      ...
      function edit(id) {
        ...
        $.ajax({
            url : "<?php echo base_url(); ?>fasilitas_uji/fasilitas_edit/" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data)
            {
              ...
              var imgs = '<?php echo base_url(); ?>upload/fasilitas_images/' + data.image;
              $("#prevew_box").attr("src", imgs);
              
              $('#modal').modal('show'); // show bootstrap modal when complete loaded
              $('.modal-title').text('Edit Fasilitas Uji'); // Set title to Bootstrap modal title
      ...
      function openModalDelete(id) {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            $.ajax({
              url: '<?php echo base_url(); ?>fasilitas_uji/fasilitas_deleted/' + id,
              type: 'DELETE',
              error: function() {
      ...
      function save()
      {
        ...
        if(save_method == 'add') {
          url = "<?php echo base_url(); ?>fasilitas_uji/fasilitas_add";
        } else {
          url = "<?php echo base_url(); ?>fasilitas_uji/fasilitas_update";
        }
      ...
      $(document).ready(function() {
        ...
        table = $('#order-table').DataTable({ 
            ...
            "ajax": {
              "url": "<?php echo base_url(); ?>fasilitas_uji/fasilitas_list",
              "type": "POST",
            },
            ...
            "aoColumns": [
              ...
              { "Image": "Image" , "sClass": "text-left" },
              { "Deskripsi": "Deskripsi" , "sClass": "text-left" },
            ],
