  ...
  <body>
          ...
          <div class="modal-body">
            <form id="RegisterValidation" enctype="multipart/form-data">
              ...
              <div class="form-group row mb-3">
                ...
                <div class="col-sm-9">
                  <input type="url" class="form-control" id="button_text" name="button_text" placeholder="Button Text">
                  <span class="help-block"></span>
                </div>
              </div>
              <div class="form-group row mb-3">
    ...
    <script>
      ...
      function openModal() {
        ...
        $('.modal-title').text('Tambah Kepercayaan'); // Set Title to Bootstrap modal title
      }

      function closeModal(){
        $('#RegisterValidation')[0].reset();
        $('#modal').modal('hide');
        $('.modal-title').text('Tambah Kepercayaan');
      }
      ...
      function edit(id) {
        ...
        $.ajax({
            url : "<?php echo base_url(); ?>trust/trust_edit/" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data)
            {
              ...
              var imgs = '<?php echo base_url(); ?>upload/trust_images/' + data.images;
              $("#prevew_box").attr("src", imgs);
              
              $('#modal').modal('show'); // show bootstrap modal when complete loaded
              $('.modal-title').text('Edit Kepercayaan'); // Set title to Bootstrap modal title
      ...
      function openModalDelete(id) {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            $.ajax({
              url: '<?php echo base_url(); ?>trust/trust_deleted/' + id,
              type: 'DELETE',
              error: function() {
      ...
      function save()
      {
        ...
        if(save_method == 'add') {
          url = "<?php echo base_url(); ?>trust/trust_add";
        } else {
          url = "<?php echo base_url(); ?>trust/trust_update";
        }
      ...
      $(document).ready(function() {
        ...
        table = $('#order-table').DataTable({ 
            ...
            "ajax": {
              "url": "<?php echo base_url(); ?>trust/trust_list",
              "type": "POST",
            },
