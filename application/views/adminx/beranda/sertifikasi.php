  ...
  <body>
          ...
          <div class="modal-body">
            <form id="RegisterValidation" enctype="multipart/form-data">
              ...
              <div class="form-group row mb-3">
                ...
                <div class="col-sm-9">
                  <input type="text" class="form-control" id="nama" name="nama" placeholder="Nama Lembaga">
                  <span class="help-block"></span>
                </div>
              </div>
              <div class="form-group row mb-3">
                <label class="col-sm-3 col-form-label">Nomor Lisensi</label>
                <div class="col-sm-9">
                  <input type="text" class="form-control" id="nomor_lisensi" name="nomor_lisensi" placeholder="Nomor Lisensi">
                  <span class="help-block"></span>
                </div>
              </div>
              <div class="form-group row mb-3">
    ...
    <script>
      ...
      function openModal() {
        ...
        $('.modal-title').text('Tambah Sertifikasi'); // Set Title to Bootstrap modal title
      }

      function closeModal(){
        $('#RegisterValidation')[0].reset();
        $('#modal').modal('hide');
        $('.modal-title').text('Tambah Sertifikasi');
      }
      ...
      function edit(id) {
        ...
        $.ajax({
            url : "<?php echo base_url(); ?>sertifikasi/sertifikasi_edit/" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data)
            {
              ...
              $('[name="nomor_lisensi"]').val(data.nomor_lisensi);
              $('[name="aktivasi"]').val(data.aktivasi);

              var imgs = '<?php echo base_url(); ?>upload/sertified_images/' + data.images;
              $("#prevew_box").attr("src", imgs);
              
              $('#modal').modal('show'); // show bootstrap modal when complete loaded
              $('.modal-title').text('Edit Sertifikasi'); // Set title to Bootstrap modal title
      ...
      function openModalDelete(id) {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            $.ajax({
              url: '<?php echo base_url(); ?>sertifikasi/sertifikasi_deleted/' + id,
              type: 'DELETE',
              error: function() {
      ...
      function save()
      {
        ...
        if(save_method == 'add') {
          url = "<?php echo base_url(); ?>sertifikasi/sertifikasi_add";
        } else {
          url = "<?php echo base_url(); ?>sertifikasi/sertifikasi_update";
        }
      ...
      $(document).ready(function() {
        ...
        table = $('#order-table').DataTable({ 
            ...
            "ajax": {
              "url": "<?php echo base_url(); ?>sertifikasi/sertifikasi_list",
              "type": "POST",
            },
            ...
            "aoColumns": [
              ...
              { "Nomor Lisensi": "Nomor Lisensi" , "sClass": "text-left" },
