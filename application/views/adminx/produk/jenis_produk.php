  ...
  <body>
          ...
          <div class="modal-body">
            <form id="RegisterValidation" enctype="multipart/form-data">
              ...
              <div class="form-group row mb-3">
                <label class="col-sm-3 col-form-label">Nama Jenis Produk</label>
                <div class="col-sm-9">
                  <input type="text" class="form-control" id="nama" name="nama" placeholder="Nama Jenis Produk">
    ...
    <script>
      ...
      function openModal() {
        ...
        $('.modal-title').text('Tambah Jenis Produk'); // Set Title to Bootstrap modal title
      }

      function closeModal(){
        $('#RegisterValidation')[0].reset();
        $('#modal').modal('hide');
        $('.modal-title').text('Tambah Jenis Produk');
      }
      ...
      function edit(id) {
        ...
        $.ajax({
            url : "<?php echo base_url(); ?>jenis_produk/jenis_produk_edit/" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data)
            {
              $('[name="kode"]').val(data.id);
              $('[name="nama"]').val(data.jenis_produk);
              $('[name="aktivasi"]').val(data.aktivasi);
              
              $('#modal').modal('show'); // show bootstrap modal when complete loaded
              $('.modal-title').text('Edit Jenis Produk'); // Set title to Bootstrap modal title
      ...
      function openModalDelete(id) {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            $.ajax({
              url: '<?php echo base_url(); ?>jenis_produk/jenis_produk_deleted/' + id,
              type: 'DELETE',
              error: function() {
      ...
      function save()
      {
        ...
        if(save_method == 'add') {
          url = "<?php echo base_url(); ?>jenis_produk/jenis_produk_add";
        } else {
          url = "<?php echo base_url(); ?>jenis_produk/jenis_produk_update";
        }
      ...
      $(document).ready(function() {
        table = $('#order-table').DataTable({ 
            ...
            "ajax": {
              "url": "<?php echo base_url(); ?>jenis_produk/jenis_produk_list",
              "type": "POST",
            },
